...
async function downloadFile(url, outPath, redirects = 0) {
  ...
  await new Promise((resolve, reject) => {
    const req = https.get(
      url,
      { headers: { "User-Agent": "claude-rs-npm-installer" } },
      (res) => {
...
async function main() {
  ...
  if (!info) {
    ...
    throw new Error(`Unsupported platform/arch for claude-rs npm install: ${key}`);
  }
  ...
  const tag = `v${version}`;
  const repo = process.env.CLAUDE_RS_RELEASE_REPO || "srothgan/claude_rust";
  const assetName = `claude-rs-${info.target}${info.exe.endsWith(".exe") ? ".exe" : ""}`;
  ...
  console.log(`Installed claude-rs ${version} (${info.target})`);
}

main().catch((error) => {
  console.error(`claude-rs postinstall failed: ${error.message}`);
  process.exit(1);
});
